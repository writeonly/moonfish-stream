...
/**
  * This is an event sourced entity. It has a state, [[MoonfishState]], which
  ...
  */
class MoonfishEntity extends PersistentEntity {

  override type Command = MoonfishCommand[_]
  override type Event = MoonfishEvent
  override type State = MoonfishState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: MoonfishState = MoonfishState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case MoonfishState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        MoonfishState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class MoonfishState(message: String, timestamp: String)

object MoonfishState {
  /**
    ...
    */
  implicit val format: Format[MoonfishState] = Json.format
}

/**
  * This interface defines all the events that the MoonfishEntity supports.
  */
sealed trait MoonfishEvent extends AggregateEvent[MoonfishEvent] {
  def aggregateTag = MoonfishEvent.Tag
}

object MoonfishEvent {
  val Tag = AggregateEventTag[MoonfishEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MoonfishEvent
...
/**
  ...
  */
sealed trait MoonfishCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends MoonfishCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends MoonfishCommand[String]
...
/**
  ...
  * application loader.
  */
object MoonfishSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MoonfishState]
  )
}
